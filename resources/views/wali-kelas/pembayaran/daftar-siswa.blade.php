...
@section('page-title', 'Input Pembayaran Siswa' . ($kelas ? ' — Kelas ' . $kelas->nama : ''))
    ...
    @else
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            <div class="divide-y divide-gray-100">
                ...
                <div x-data="{ open: false }"
                    ...
                    <div x-show="open" x-cloak class="bg-gray-50 border-t border-gray-100 px-4 py-3 space-y-3">
                        ...
                        @else
                            ...
                            @php $icon = $kategoriIcon[$tagihan->jenisTagihan->kategori] ?? ['icon'=>'fa-tag','color'=>'text-gray-400']; @endphp

                            <div x-data="{ showVoid: null }" class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                                ...
                                <div class="flex items-center gap-3 px-3 py-2.5">
                                    ...
                                    {{-- Progress --}}
                                    @php $pct = $tagihan->nominal_total > 0 ? min(100, round($tagihan->nominal_terbayar / $tagihan->nominal_total * 100)) : 0; @endphp
                                    ...
                                    <a href="{{ route('wali-kelas.pembayaran.form-tunai', $tagihan->id) }}"
                                       class="flex-shrink-0 text-xs bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg">
                                        Bayar
                                    </a>
                                    @else
                                ...
                                <div class="border-t border-gray-100 px-3 py-2 bg-gray-50">
                                    ...
                                    <div class="space-y-1.5">
                                        ...
                                        <div class="flex items-center justify-between text-xs" x-data="{ showVoidForm: false }">
                                            <div class="flex items-center gap-2 flex-wrap">
                                                ...
                                                <span class="text-gray-400">· {{ \App\Models\Pembayaran::metodeLabel()[$bayar->metode] }}</span>
                                            ...
                                            <button type="button"
                                                    ...
                                                    class="text-red-400 hover:text-red-600 hover:underline ml-2 flex-shrink-0">
                                                <i class="fas fa-undo-alt mr-1"></i>Batalkan
                                            </button>
                                        </div>

                                        {{-- Form void --}}
                                        <div x-show="showVoidForm" x-cloak
                                             class="bg-red-50 border border-red-200 rounded-lg p-2 mt-1">
                                            <p class="text-xs text-red-700 font-medium mb-1.5">
                                            ...
                                            <form method="POST" action="{{ route('wali-kelas.pembayaran.void', $bayar->id) }}"
                                                ...
                                                <input type="text" name="catatan_void" required
                                                       ...
                                                       class="flex-1 border border-red-300 rounded px-2 py-1 text-xs focus:ring-1 focus:ring-red-400 outline-none">
                                                <button type="submit"
                                                        class="bg-red-600 hover:bg-red-700 text-white text-xs px-3 py-1 rounded flex-shrink-0">
                                                    Konfirmasi
                                                </button>
                                                <button type="button" @click="showVoidForm = false"
                                                        class="bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs px-2 py-1 rounded flex-shrink-0">
                                                    Batal
                                                </button>
                                            </form>
                                        </div>
                ...
                <div x-show="!$el.previousElementSibling || {{ $siswa->count() }} === 0" class="hidden">
                </div>
                <p class="text-center text-sm text-gray-500 py-6"
                   x-show="search && !document.querySelector('[x-show*=matches]:not([style*=none])')">
